...
{
    public List<Location> FindBy(LocationCriteria criteria)
    {
        ...
        {
            joins.Add("JOIN PositionLocation ON PositionLocation.LocationId = Location.id");
            conditions.Add(new SqlCondition("PositionLocation.PositionId", criteria.PositionId));
        }
        ...
        {
            Debug.WriteLine(sql);
            locations = conn.Query<Location>(sql, queryParameters.parameters).ToList();
        }
        return locations;
    }
}
